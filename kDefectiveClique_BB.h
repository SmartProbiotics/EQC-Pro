...
private:
	...
	ui degeneracy_ordering_and_coloring_adj(ui S_end, ui R_end, ui level, ui *color) {
		ui *rid = buf;       // 存放每个顶点在排序中的位置
		ui *id  = buf1;      // 计数排序数组，最终存放按度数排序的顶点
		ui *t_degree = color; // 暂存各顶点的度数

		ui max_degree = 0;

		// 计算 [S_end, R_end) 内每个顶点的度数（仅统计 removed_level >= level 的边）
		for (ui i = S_end; i < R_end; i++) {
			ui vertex = SR[i];
			ui d = 0;
			for (ui j = pstart_R[vertex]; j < pend[vertex]; j++) {
				if (removed_level[edges[j]] < level)
					break;
		...
		for (ui i = R_end - S_end; i > 0; i--) {
			ui u = id[i - 1];
			for (ui k = pstart_R[u]; k < pend[u]; k++) {
				if (removed_level[edges[k]] < level)
					break;
				ui edge = edges[k];
				if (SR_rid[edge] < R_end && rid[edge] >= i)
					vis[color[edge]] = 1;
			}
			// 选择最小可用颜色
			ui c = 0;
    ...
    bool collect_removable_vertices_based_on_degree_in_S_opt(ui S_end, ui R_end, ui level) {
    	...
    	for(ui i = 0;i < R_end-S_end&&remaining_vertices_n;i ++) {
    		...
    		if(t_missing_edges+R_end-S_end-1+degree_in_S[u]-degree[u] > K) {
    			...
    			else { //S, u,v1,v2,...,C1(with vr where r=i+remaining_vertices_n-ids_n),C2
    				ui cut_off = degree_in_S[candidates[i+remaining_vertices_n-ids_n]];
    				for(ui j = pstart_R[u];j < pend[u]&&removed_level[edges[j]] >= level;j ++) if(removed_level[edges[j]] > level) {
    					if(degree_in_S[edges[j]] > cut_off) ++ n_neighbors_more;
    					if(degree_in_S[edges[j]] == cut_off) {
    						if(SR_rid[edges[j]] <= i+remaining_vertices_n-ids_n) ++ n_neighbors_same_lower; //link to u, but in C1
    						else ++ n_neighbors_same_higher; //link to u, but in C2
    					}
    				}
    			...
    			if(n_neighbors_same_lower >= t_n_same_lower){
					std::cout<<"n_neighbors_same_lower: "<<n_neighbors_same_lower<<", t_n_same_lower: "<<t_n_same_lower<<",	n_neighbors_same_higher: "<<n_neighbors_same_higher<<", n_neighbors_more: "<<n_neighbors_more<<std::endl;
    ...
    void restore_R(ui S_end, ui &R_end, ui old_R_end, ui level) {
        ...
        while(R_end < old_R_end) { // insert u back into R
            ...
            for(ui i = pstart[u];i < pend[u]&&removed_level[edges[i]] >= level;i ++) {
				...
				// ui v=edges[i];
				// degree_in_S[v]=0;
				// for(ui j = pstart[v];j < pend[v]&&removed_level[edges[j]] >= level;j ++) if(SR_rid[edges[j]] < S_end) {
				// 	++ degree_in_S[v];
				// }
            }
