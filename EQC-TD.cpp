...
void greedyColoring(Graph* graph, const ui* peel_sequence, int* &color_cnt, int &max_color) {
    ...
    for (int idx = 0; idx < n; ++idx) {
        ...
        for (ept ei = pstart[u]; ei < pstart[u + 1]; ++ei) {
            int v = edges[ei];
            int c = color[v];
            if (c) forbiddenTime[c] = curTime;
        }
        int c;
        for (c = 1; c <= max_color; ++c) {
            if (forbiddenTime[c] != curTime) break;
        }
        color[u] = c;
...
void extend(double alpha, int k_cur, vector<ui> &result, ui *&to_static_mapping, ui *&out_mapping, int &lb, bool do_first_heuris_n = false)
{
    ...
    while (1)
    {
        ...
        {
            //./kDC ~/Program/DATA/realworld_graphs/soc/soc-twitter-follows.bin 0.83 #great improvement!
            k_cur++;
...
int main(int argc, char *argv[])
{
    ...
    for(int i=0;i<max_color;i++) cout<<color_cnt[i]<<" ";
    delete[] color_cnt;
    vector<ui> deg;
    for(int i=0;i<static_g.n;i++) deg.push_back(static_g.pstart[i+1]-static_g.pstart[i]);
    ...
    while(deg_ub!=deg_lb){
        ...
        ui sum=0;
        for(int i=0;i<mid;i++) sum+=min(deg[i],mid-1);
        // cout<<"Mid: "<<mid<<" Sum: "<<sum<<" Need: "<<my_floor(gama*mid*(mid-1))<<endl;
        if(sum>=my_floor(gama*mid*(mid-1))) deg_lb=mid;
        else deg_ub=mid-1;
    }

    cout<<"Deg UB: "<<deg_ub<<endl;
    if(deg_ub<ub) ub=deg_ub;
